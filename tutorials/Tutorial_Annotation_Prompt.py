...
import json
import os
# Fix for PermissionError with Numba in read-only environments
# This redirects Numba's cache to a local folder instead of the system-wide site-packages
os.environ['NUMBA_CACHE_DIR'] = os.path.join(os.getcwd(), ".numba_cache")
# Use reasonable thread counts for linear algebra
...
try:
    ...
    # Workaround for broken torchaudio installation on Windows
...
# Performance and memory balance
torch.set_num_threads(4)  # Use 4 threads for good balance between speed and memory
torch.cuda.per_process_memory_fraction = 0.9  # Allow up to 90% of VRAM
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_model", type=str, default=None, help="Path to the pretrained model directory")
    ...
    hyperparameter_defaults = dict(
        ...
        do_train=True,
        load_model=args.load_model if args.load_model else str(project_root / "scGPT_human"),
        mask_ratio=0.0,
        epochs=args.epoch,
        n_bins=51,
    ...
    if dataset_name == "ms":
        ...
        if config.subset_train is not None:
            ...
            print(f"Subsetted training data to {config.subset_train} cells")
        if adata.n_obs == 0:
            raise ValueError("Training data is empty.")
        if adata.n_vars == 0:
            raise ValueError("Training data has no genes.")
        try:
            adata_val = sc.read(data_dir / "ms_val.h5ad")
            # Subset validation data if requested
            if config.subset_val is not None and adata_val is not None:
                adata_val = adata_val[:config.subset_val, :].copy()
                print(f"Subsetted validation data to {config.subset_val} cells")
            if adata_val is not None and adata_val.n_obs == 0:
                print("Warning: Validation set is empty. Proceeding without validation set.")
                adata_val = None
        ...
        if config.subset_test is not None:
            ...
            print(f"Subsetted test data to {config.subset_test} cells")
        if adata_test.n_obs == 0:
            raise ValueError("Test data is empty.")
        if "celltype" not in adata.obs:
            raise ValueError("Training data missing 'celltype' column.")
        if "celltype" not in adata_test.obs:
            raise ValueError("Test data missing 'celltype' column.")
        if adata_val is not None and "celltype" not in adata_val.obs:
            print("Warning: Validation data missing 'celltype' column. Dropping validation set.")
            adata_val = None
        ...
        if config.subset_train is not None:
            adata = adata[:config.subset_train, :].copy()
            print(f"Subsetted training data to {config.subset_train} cells")
        # Set gene symbol as index and make unique for train
        print('Train var columns:', adata.var.columns)
        adata.var['ensembl_id'] = adata.var.index
        adata.var['feature_name'] = adata.var['feature_name'].astype('object')  # Fix chính ở đây!
        adata.var_names = adata.var['feature_name']  # Change 'feature_name' if your symbol column is named differently
        adata.var_names_make_unique()
        try:
            adata_val = sc.read(data_dir / "cxg_validation.h5ad")
            # Subset validation data if requested
            if config.subset_val is not None:
                adata_val = adata_val[:config.subset_val, :].copy()
                print(f"Subsetted validation data to {config.subset_val} cells")
            # Set gene symbol as index and make unique for val
            print('Val var columns:', adata_val.var.columns)
            adata_val.var['ensembl_id'] = adata_val.var.index
            adata_val.var['feature_name'] = adata_val.var['feature_name'].astype('object')  # Fix chính ở đây!
            adata_val.var_names = adata_val.var['feature_name']
        ...
        if config.subset_test is not None:
            adata_test = adata_test[:config.subset_test, :].copy()
            print(f"Subsetted test data to {config.subset_test} cells")
        # Set gene symbol as index and make unique for test
        print('Test var columns:', adata_test.var.columns)
        adata_test.var['ensembl_id'] = adata_test.var.index
        adata_test.var['feature_name'] = adata_test.var['feature_name'].astype('object')  # Fix chính ở đây!
        adata_test.var_names = adata_test.var['feature_name']
        adata_test.var_names_make_unique()

        if adata.n_obs == 0: raise ValueError("Training data is empty.")
        if adata_test.n_obs == 0: raise ValueError("Test data is empty.")
        if "cell_type" not in adata.obs: raise ValueError("Training data missing 'cell_type' column.")
        if "cell_type" not in adata_test.obs: raise ValueError("Test data missing 'cell_type' column.")
        if adata_val is not None and "cell_type" not in adata_val.obs:
            print("Warning: Validation data missing 'cell_type' column. Dropping validation set.")
            adata_val = None
    ...
    celltype_dict = dict(enumerate(adata.obs["celltype"].astype("category").cat.categories))
    ...
    if config.load_model is not None:
        ...
        logger.info(f"First 10 genes in data: {adata.var['gene_name'].tolist()[:10]}")
        logger.info(f"First 10 genes in vocab: {list(vocab.get_stoi().keys())[:10]}")
        if np.sum(gene_ids_in_vocab >= 0) == 0:
            logger.warning("No genes matched. Trying to match by capitalizing gene names...")
            adata.var["id_in_vocab"] = [
                1 if str(gene).upper() in vocab else -1 for gene in adata.var["gene_name"]
            ]
            gene_ids_in_vocab = np.array(adata.var["id_in_vocab"])
            if np.sum(gene_ids_in_vocab >= 0) > 0:
                logger.info(
                    f"match {np.sum(gene_ids_in_vocab >= 0)}/{len(gene_ids_in_vocab)} genes "
                    f"in vocabulary of size {len(vocab)} after capitalization."
                )
                adata.var["gene_name"] = [str(gene).upper() for gene in adata.var["gene_name"]]
            else:
                logger.error(f"First 10 genes in data: {adata.var['gene_name'].tolist()[:10]}")
                logger.error(f"First 10 genes in vocab: {list(vocab.get_stoi().keys())[:10]}")

        adata = adata[:, adata.var["id_in_vocab"] >= 0]

        if adata.n_vars == 0:
            raise ValueError(
                "No genes matched the vocabulary. Please check if the gene names in the data match the vocabulary."
            )
...
def prepare_dataloader(
    ...
    # Use up to 4 workers for parallel data loading (good balance for most systems)
    # Increase to 6-8 if you have >16GB RAM and many CPU cores. Decrease to 0-2 if memory issues occur.
    ...
    if per_seq_batch_sample:
        ...
        data_loader = DataLoader(
            ...
            pin_memory=True,  # Enable for GPU speed
        )
        return data_loader
...
def evaluate(model: nn.Module, loader: DataLoader, return_raw: bool = False) -> float:
    ...
    if total_num > 0:
        ...
        val_loss, val_err = total_loss / total_num, total_error / total_num
    else:
...
def test(model: nn.Module, adata: DataLoader) -> float:
    ...
    test_loader = DataLoader(
        ...
        shuffle=False,
        drop_last=False,
        num_workers=2,  # Use 2 workers for reasonable speed
        pin_memory=True,  # Enable for GPU speed
    )
...
if __name__ == "__main__":
    ...
    target_names = [id2type.get(l, f"unknown_label_{l}") for l in unique_labels]
    logger.info(classification_report(labels, predictions, labels=unique_labels,
