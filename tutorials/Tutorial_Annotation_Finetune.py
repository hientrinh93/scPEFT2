...
def prepare_data(sort_seq_batch=False) -> Tuple[Dict[str, torch.Tensor]]:
    if mask_ratio > 0:
        ...
        masked_values_train = tokenized_train["values"]
        masked_values_valid = tokenized_valid["values"]
...
def prepare_dataloader(
    ...
    # Modified to respect num_workers=0 (main thread) if requested, 
    # preventing crashes on systems with limited resources or Windows.
    # if num_workers == 0:
    #     if hasattr(os, "sched_getaffinity"):
    #         num_cpus = len(os.sched_getaffinity(0))
    #     else:
    #         num_cpus = os.cpu_count() or 1
    #     num_workers = min(num_cpus, batch_size // 2)
...
def test(model: nn.Module, adata: DataLoader) -> float:
    ...
    test_loader = DataLoader(
        ...
        shuffle=False,
        drop_last=False,
        num_workers=min(num_cpus, eval_batch_size // 2),
        pin_memory=True,
    )
...
if __name__ == "__main__":
    ...
    if hasattr(args, 'load_model'):
        if args.load_model is not None and not os.path.isabs(args.load_model):
            args.load_model = str((script_dir / args.load_model).resolve())
    else:
        args.load_model = str((script_dir / '../scGPT_human').resolve())
    ...
    if dataset_name == "ms":
        ...
        train_file = data_dir / "ms_train.h5ad"
        if not train_file.exists():
            raise FileNotFoundError(f"Data file not found: {train_file}. Please download the data.")
    ...
    # Removed global all_counts to save memory (it was unused in this scope)
    # all_counts = (
    #     adata.layers[input_layer_key].A
    #     if issparse(adata.layers[input_layer_key])
    #     else adata.layers[input_layer_key]
