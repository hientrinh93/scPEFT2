...
class CellEmbDataset(torch.utils.data.Dataset):
...
class CellEmbRankDataset(torch.utils.data.Dataset):
    def __init__(self, count_matrix, all_raw_counts, gene_ids, batch_ids=None, vocab=None, model_configs=None):
        self.count_matrix = count_matrix
...
def get_batch_cell_embeddings(
    ...
    if cell_embedding_mode == "cls":
        ...
        # On Windows, multiprocessing spawn can cause issues; use single-process data loading.
...
def get_batch_cell_embeddings_rank(
    ...
    if use_batch_labels:
        ...
        dataset = CellEmbRankDataset(
            count_matrix, all_raw_counts, gene_ids, batch_ids if use_batch_labels else None, vocab=vocab, model_configs=model_configs
        )
