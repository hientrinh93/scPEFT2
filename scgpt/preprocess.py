...
class Preprocessor:
    ...
    def __call__(self, adata: AnnData, batch_key: Optional[str] = None) -> Dict:
        ...
        """
        if adata.n_vars == 0:
            logger.warning("AnnData has no genes. Preprocessing skipped.")
            return {}
        ...
        if self.binning:
            ...
            if layer_data.size > 0 and layer_data.min() < 0:
                raise ValueError(
            ...
            for row in layer_data:
                if row.size == 0:
                    ...
                    bin_edges.append(np.zeros(n_bins))
                    continue
                if row.max() == 0:
                    logger.warning(
            ...
            if len(binned_rows) > 0:
                ...
                adata.layers[self.result_binned_key] = np.empty((0, layer_data.shape[1]))
                adata.obsm["bin_edges"] = np.empty((0, n_bins))
    ...
    def check_logged(self, adata: AnnData, obs_key: Optional[str] = None) -> bool:
        ...
        # Handle empty data
        if (hasattr(data, 'shape') and (data.shape[0] == 0 or data.shape[1] == 0)) or data.size == 0:
            logger.warning("Input data is empty in check_logged. Returning False.")
            return False
...
def binning(
    ...
    # handle empty row
    if row.size == 0:
        binned_row = np.empty_like(row, dtype=np.int64)
    ...
    # handle presence of zeros but some non-zero entries
